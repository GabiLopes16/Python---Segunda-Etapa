...
@api_v1_bp.route("/locacoes", methods=["GET"]) #TODO Qual é a rota?                  api_v1_bp
...
def api_detalhe_veiculo(veiculo_id):
    ...
    if not veiculo:
        return jsonify({"erro 404": "Veículo não encontrado"}), #TODO QUAL ERRO APARECE QUANDO NÃO ENCONTRA O VEÍCULO??             404
    return jsonify({
        "id": veiculo.id,
        "placa": veiculo.placa,
...
def api_modelos_nhtsa():
    marca = request.args.get("marca", "Honda") #TODO responda por que estamos utilizando o request.args.get?            para retornar um dic multivalorado imutavel
    try:
